    ...
    if (_vtx_freq_change_pending || _vtx_power_change_pending || _vtx_options_change_pending) {
        ...
        if (pitMode != pitModeRunning) {
            if (pitModeRunning) {
                ...
                // turn it on
                mode = ((opts & uint8_t(AP_VideoTX::VideoOptions::VTX_OUTRANGE_PITMODE)) ? 0x02 : 0x01)
                    | ((opts & uint8_t(AP_VideoTX::VideoOptions::VTX_UNLOCKED)) << 2);
            }
        } else {
        ...
        if (_vtx_freq_change_pending) {
            ...
            if (_protocol_version == SMARTAUDIO_SPEC_PROTOCOL_v21) {
...
void AP_SmartAudio::update_vtx_settings(const Settings& settings)
{
    ...
    // PITMODE | UNLOCKED | OUTRANGE
    uint8_t opts = ((settings.mode & 0x2) >> 1) | ((settings.mode & 0x10) >> 3) | ((settings.mode & 0x8) >> 1);
...
bool  AP_SmartAudio::parse_response_buffer(const uint8_t *buffer)
{
    ...
    case SMARTAUDIO_RSP_SET_POWER: {
        ...
        vtx.set_power_level(resp->payload & 0xFF);
        debug("Power was set to to %d", resp->payload & 0xFF);
    }
        break;
